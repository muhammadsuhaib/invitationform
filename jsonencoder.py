# encoding=utf-8
#
# found here http://www.traddicts.org/webdevelopment/flexible-and-simple-json-serialization-for-django/
# added datetime conversion by Daniel Kurashige-Gollub, July 2012
#
...
class JSONSerializer():
    ...
    def handle_object(self, object):
        ...
        if object is None:
        ...
        elif isinstance(object, list) or type(object) == type(()):
            ...
            self.handle_simple(object.isoformat())
    ...
    def handle_dictionary(self, d):
        ...
        for key, value in d.iteritems():
            self.currentLoc += unicode(key) + '.'
            # self.stream.write(unicode(self.currentLoc))
            i += 1
            self.handle_simple(unicode(key))
            self.stream.write(u': ')
            self.handle_object(value)
            if i != len(d):
                self.stream.write(u', ')
            self.currentLoc = self.currentLoc[0:(len(self.currentLoc) - len(unicode(key)) - 1)]
        self.end_object()
